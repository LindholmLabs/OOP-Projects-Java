...
public class CoffeeMaker extends Thread {
	...
	/**
	 * TimerTask used for creating coffee. Each cup is added to the coffeeBuffer
	 * where a worker then can retrieve it.
	 */
	...
	/**
	 * Timertask serveCoffee. Serves the first worker in the queue with the next cup
	 * of coffee from the coffeeBuffer; a concurrentLinkedQueue containing all cups
	 * of coffee the machine has created.
	 */
		...
		public void run() {
			if (coffeeQueue.getSize() != 0 && coffeeBuffer.size() != 0) {
				...
				if (worker.getEnergy() < maxWorkerEnergy) {
					worker.drink(coffee);
					System.out.println(worker.getWorkerName() + " enjoyed a " + coffee.getType() + " with "
							+ coffee.getEnergy() + " energy");
				}
			}
		}
	};
	...
	/**
	 * Run thread. Start serveCoffee and CreateCoffee schedules.
	 */
	public void run() {
	 ...
	 * 
	 * @param coffeeQueue The queue that workers use to interface with the
	 *                    coffeemaker.
	 */
	...
	/**
	 * Generate a random coffee. Each coffee gets a random energy value depending on
	 * type.
	 * 
